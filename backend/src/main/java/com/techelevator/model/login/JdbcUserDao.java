...
public class JdbcUserDao implements UserDao {
     ...
     */
    @Override
    public User saveUser(String userName, String password, String role, int familyID) {
        byte[] salt = passwordHasher.generateRandomSalt();
        String hashedPassword = passwordHasher.computeHash(password, salt);
        String saltString = new String(Base64.getEncoder().encode(salt));
        long newId = jdbcTemplate.queryForObject(
                "INSERT INTO users(username, password, salt, role, family_id) VALUES (?, ?, ?, ?, ?) RETURNING id", Long.class,
                userName, hashedPassword, saltString, role, familyID);
    ...
    public List<User> getAllUsers() {
        ...
        String sqlSelectAllUsers = "SELECT id, username, role, family_id FROM users";
    ...
    @Override
    public List<User> getFamiliesChildren(String role, int familyId) {
        List<User> children = new ArrayList<User>();
        String sqlSelectAllUsers = "SELECT id, username, role, family_id " +
                                   "FROM users " + 
                                   "WHERE role = ? AND family_id = ?;";
        SqlRowSet results = jdbcTemplate.queryForRowSet(sqlSelectAllUsers, role, familyId);
    ...
    private User mapResultToUser(SqlRowSet results) {
        ...
        user.setFamilyId(results.getInt("family_id"));
        return user;
    }

    @Override
    public User getUserByUsername(String username) {
        String sqlSelectUserByUsername = "SELECT id, username, role, family_id FROM users WHERE username = ?";
    ...
    /**
     * Get all of the users from the database by family ID
     * @param familyId the family Id of the user- see the families tables
     * @return a List of user objects
     * ---------------------------------------
     * History
     * S.Stier 4/10/2020 Added to for Setup Family Accounts
     */
    @Override
    public List<User> getAllUsersByFamilyId(int familyId) {
        List<User> users = new ArrayList<User>();
        String sqlSelectAllUsers = "SELECT id, username, role, family_id FROM users where family_id = ?";
        SqlRowSet results = jdbcTemplate.queryForRowSet(sqlSelectAllUsers, familyId);
    ...
    public User getUserByUserId(int id) {
        String sqlSelectUserByUsername = "SELECT id, username, role, family_id FROM users WHERE id = ?";
